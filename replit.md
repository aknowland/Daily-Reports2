...
-   **Contract Gantt Chart**: Interactive timeline visualization at the top of the Contract Management page showing all contracts with "Awarded" or "In Execution" status. Features dual-color bars (blue for schedule progress, green/amber/red for budget progress), a red dashed "Today" marker, zoom controls, KPI summary (counts, total budget, overdue alerts), clickable bars/labels for navigation to contract dashboards, responsive label column width, and color-coded legend. Uses `/api/contracts/dashboard-summary` for accurate budget data with client-side fallback.
