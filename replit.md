...
-   **Custom GPT / Claude Actions API**: Company admins generate API keys to connect ChatGPT Custom GPTs or Claude Projects to their company's data. A full read/write REST API (`/api/v1/`) is exposed for projects (create/update with budget & schedule), contracts (create/update), clients (create/update), IOR agreements (create/update), team invites (send), plus read-only reports and summary. An OpenAPI spec is served at `/api/v1/openapi.json` for use in ChatGPT or Claude's Action/Tool setup. All endpoints are secured with Bearer token (API key) auth.
-   **Inspector Recruiting Module**: Imports DSA-certified inspector lists (Class 1/2/3) from the California Division of the State Architect registry. Company admins can track recruiting status (Prospect → Contacted → Interested → Hired), filter by class/county/status, and log timestamped notes per candidate. "Add to Team" promotion button for hired inspectors. DB tables: `inspector_candidates`, `inspector_candidate_notes`. Routes: `/api/recruiting/*`. Page: `/company/recruiting`.
