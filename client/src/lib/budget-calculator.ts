...
export function calculateHybridBudget(
  ...
  // Pass hasBaseBudget to use January 2, 2026 as start date when base budget exists
  const hasBaseBudget = baseBudgetNum > 0;
  const scheduledDetails = calculateScheduledBudget(startDate, endDate, inspectors, asOfDate, hasBaseBudget);
